...
def interpolate(ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, directions, flagD):
...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    for (m, l, i) in zip(mode, lengths, list(range(len(mode)))):
        if l > 0.0:
            d = step_size
            flagD = 1
        else:
        ...
        while abs(pd) <= abs(l):
            ...
            px, py, pyaw, directions = interpolate(
                ind, pd, m, maxc, ox, oy, oyaw, px, py, pyaw, directions, flagD)
            pd += d
        ...
        ind += 1
        px, py, pyaw, directions = interpolate(
            ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, directions, flagD)
